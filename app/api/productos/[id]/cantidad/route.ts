...
export async function PATCH(request: Request, { params }: { params: { id: string } }) {
  try {
    ...
    if (cambio === undefined) {
      return NextResponse.json({ error: "El cambio de cantidad es obligatorio" }, { status: 400 })
    }

    // Obtener cantidad actual
    const currentResult = await query("SELECT cantidad FROM productos WHERE id = $1", [id])
