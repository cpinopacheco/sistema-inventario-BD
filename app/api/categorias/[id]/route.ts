...
export async function PUT(request: Request, { params }: { params: { id: string } }) {
  try {
    ...
    const categoryCheck = await query("SELECT * FROM categorias WHERE id = $1", [id])
    ...
    const nameCheck = await query("SELECT * FROM categorias WHERE nombre = $1 AND id != $2", [nombre, id])

    if (nameCheck.rowCount > 0) {
      return NextResponse.json({ error: "Ya existe otra categoría con ese nombre" }, { status: 409 })
    }

    const result = await query("UPDATE categorias SET nombre = $1 WHERE id = $2 RETURNING *", [nombre, id])
...
export async function DELETE(request: Request, { params }: { params: { id: string } }) {
  try {
    ...
    const productsCheck = await query("SELECT COUNT(*) FROM productos WHERE categoria_id = $1", [id])
