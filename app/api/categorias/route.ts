...
export async function GET() {
  try {
    ...
    return NextResponse.json(
      { error: "Error al obtener categorías: " + (error instanceof Error ? error.message : "Error desconocido") },
      { status: 500 },
    )
  }
}
...
export async function POST(request: Request) {
  try {
    ...
    const existingCategory = await query("SELECT * FROM categorias WHERE nombre = $1", [nombre])
    ...
    return NextResponse.json(
      { error: "Error al crear categoría: " + (error instanceof Error ? error.message : "Error desconocido") },
      { status: 500 },
    )
  }
}
