...
export default function ProductDetail({
  ...
  useEffect(() => {
    if (onUpdateQuantity === undefined || typeof onUpdateQuantity !== "function") {
      console.warn("ProductDetail: onUpdateQuantity no es una función o no está definida")
    }
  }, [onUpdateQuantity])
  ...
  const handleCategoryChange = (value: string) => {
    if (value === "nueva") {
      ...
      const selectedCategory = categories.find((c: Categoria) => c.id.toString() === value)
      setEditedProduct({
        ...editedProduct,
  ...
  const handleAddCategory = async () => {
    ...
    const existingCategory = categories.find((c: Categoria) => c.nombre.toLowerCase() === newCategory.toLowerCase())
  ...
  const handleEditCategory = async () => {
    ...
    const existingCategory = categories.find(
      (c: Categoria) =>
        c.nombre.toLowerCase() === categoryToEdit.new.toLowerCase() && c.nombre !== categoryToEdit.original,
    )
          ...
          <CardContent className="pt-4 bg-background px-6">
            {isLoading ? (
              <div className="flex flex-col items-center justify-center py-8">
                ...
                <p className="text-sm text-gray-500">Cargando detalles del producto...</p>
              </div>
            ) : isEditing ? (
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <Select value={editedProduct.categoria_id.toString()} onValueChange={handleCategoryChange}>
                    ...
                    <SelectContent className="bg-background">
                      ...
                      {product.categoria && (
                        <SelectItem value="editar" className="text-amber-600 dark:text-amber-400 font-medium group">
                          <div className="flex items-center gap-1 group-hover:text-black">
                            <Edit2 size={14} />
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex items-center gap-2">
                    <div className="flex-1 text-sm font-medium">{product.cantidad}</div>
                    <div className="flex gap-1">
                      <TooltipSimple text="Aumentar cantidad">
                        <Button
                          variant="outline"
                          size="icon"
                          className="h-8 w-8 text-green-600 border-green-500 hover:bg-green-100 dark:text-green-400 dark:border-green-500 dark:hover:bg-green-900 border-2"
                      ...
                      <TooltipSimple text="Disminuir cantidad">
                        <Button
                          variant="outline"
                          size="icon"
                          className="h-8 w-8 text-gray-600 border-gray-400 hover:bg-gray-100 dark:text-gray-300 dark:border-gray-500 dark:hover:bg-gray-800 border-2"
              ...
              >
                    ...
                    <div className="flex gap-1">
                      <TooltipSimple text="Aumentar cantidad">
                        <Button
                          variant="outline"
                          size="icon"
                          className="h-6 w-6 text-green-600 border-green-500 hover:bg-green-100 dark:text-green-400 dark:border-green-500 dark:hover:bg-green-900 border-2"
                          onClick={() => handleOpenAdjustmentModal(true)}
                        >
                          <Plus size={12} />
                        </Button>
                      ...
                      <TooltipSimple text="Disminuir cantidad">
                        <Button
                          variant="outline"
                          size="icon"
                          className="h-6 w-6 text-gray-600 border-gray-400 hover:bg-gray-100 dark:text-gray-300 dark:border-gray-500 dark:hover:bg-gray-800 border-2"
                          onClick={() => handleOpenAdjustmentModal(false)}
                          disabled={product.cantidad <= 0}
                        >
                          <Minus size={12} />
                ...
                <div className="space-y-1">
                  ...
                  <div
                    className={`text-sm font-medium ${product.cantidad > 10 ? "text-green-600 dark:text-green-400" : "text-red-600 dark:text-red-400"
                      }`}
                  >
            ...
            {isLoading ? (
              ...
              <>
                <TooltipSimple text="Editar producto">
                  <Button
                    variant="outline"
                    className="text-blue-600 border-blue-500 hover:bg-blue-50 dark:text-blue-400 dark:border-blue-500 dark:hover:bg-blue-950 border-2"
                ...
                <TooltipSimple text="Eliminar producto">
                  <Button
                    variant="outline"
                    className="text-red-600 border-red-500 hover:bg-red-50 dark:text-red-400 dark:border-red-500 dark:hover:bg-red-950 border-2"
